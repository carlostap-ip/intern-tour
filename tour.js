...
const GLOSSARY = {
  ...
  sqlite: { term: 'SQLite', short: 'A tiny embedded database — the whole DB is one file, no server process. Row-oriented with B-tree indexes, so it jumps straight to a row by key. We use it on the serve box for hot point lookups: patent-by-number, family, and citation edges. It is the "lean lookup": the bulky abstract is left out and fetched from Parquet only when needed, which keeps the file small.' },
